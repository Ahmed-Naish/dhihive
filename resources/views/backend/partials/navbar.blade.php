        ...
        <div class="header-control-item">
            <div class="item-content dropdown">
                <a class="nav-link __clock_nav mt-0" href="javascript:void(0)">
                    <span class="clock company_name_clock fs-16" id="clock"
                        onload="currentTime()">{{ _trans('partial.00:00:00') }}</span>
                </a>
            </div>
        </div>
        ...
        @if (!isMainCompany())
            <div class="header-control-item">
                ...
                <div class="item-content d-flex align-items-center">
                    <div class="mt-0 d-flex  align-items-between pe-auto cursor-pointer pointer " role="navigation"
                        id="topbar_messages" aria-expanded="false">
                        @if (!isAttendee()['checkin'])
                            <button type="button" id="demo"
                                onclick="viewModal(`{{ route('admin.ajaxDashboardCheckinModal') }}`)"
                                data-bs-toggle="tooltip" data-bs-placement="right"
                                data-bs-title="{{ _trans('common.Check In') }}"
                        ...
                        @if (isAttendee()['checkin'] && !isAttendee()['checkout'])
                            <button type="button"
                                onclick="viewModal(`{{ route('admin.ajaxDashboardCheckOutModal') }}`)"
                                data-bs-toggle="tooltip" data-bs-placement="right"
                                data-bs-title="{{ _trans('common.Check Out') }}"
                            ...
                            <input type="text" hidden value="{{ url('assets/coffee-break.png') }}" id="break_icon">
                            <span class="break_back_button">
                                @if (isBreakRunning() == 'start')
                                    <a onclick="breakBack(`{{ route('admin.ajaxDashboardBreakModal_Back') }}`, `{{ route('admin.ajaxDashboardBreakModal') }}`)"
                                        data-bs-toggle="tooltip" data-bs-placement="right"
                                        data-bs-title="{{ _trans('common.Break') }}"
            ...
            <div class="header-control-item d-none d-lg-block">
                ...
                <div class="item-content">
                    <button class="mt-0 noti-color sm-btn-with-radius position-relative " type="button"
                        id="topbar_notifications" data-bs-toggle="dropdown" aria-expanded="false">
                        <span class="noti-btn " data-bs-toggle="tooltip" data-bs-placement="bottom"
                        ...
                        <div class="topbar-dropdown-content">
                            ...
                            @forelse (unreadNotification()  as $key => $notification)
                                @if (!empty($notification))
                                    <a class="dropdown-item topbar-dropdown-item d-flex align-items-start"
                                        ...
                                        <div class="d-flex flex-column">
                                            ...
                                            <div class="item-status">
                                                <span class="time"> {{ @$notification->created_at->diffForHumans() }}
                                                </span>
                ...
                <div class="item-content">
                    <button class="mt-0 noti-color sm-btn-with-radius position-relative " type="button"
                        ...
                        <span class="noti-btn " data-bs-toggle="tooltip" data-bs-placement="right"
                            data-bs-title="{{ _trans('common.Expire Notification') }}"> <i
                                class="las la-bell text-danger"></i></span>
                    ...
                    <div class="dropdown-menu dropdown-menu-end topbar-dropdown-menu ot-card pv-32 ph-16 topbar_notifications"
                        ...
                        <div class="topbar-dropdown-content notification_body">
                            @forelse (expireNotification()  as $notification)
                                <a class="dropdown-item topbar-dropdown-item d-flex align-items-start"
                                    href="{{ route('notification-read', [$notification->id, $notification->employee_id]) }}">
                                    <div class="d-flex flex-column">
                                        <div class="d-flex gap-3 flex-row">
                        ...
                        <div class="profile-expand-list d-flex flex-column">
                            ...
                            @if (!isMainCompany())
                                <a class="profile-expand-item" href="{{ route('notification.index') }}"
                                    data-bs-toggle="tooltip" data-bs-placement="right"
                                    data-bs-title="{{ _trans('common.Notification') }}">
