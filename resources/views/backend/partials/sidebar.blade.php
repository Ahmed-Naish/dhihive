    ...
    <div class="sidebar-header">
        <div class="sidebar-logo">
            <a href="{{ route('admin.dashboard') }}" class="img-tag sidebar_logo">
                ...
                <img class="half-logo" src="{{ uploaded_asset(@base_settings('company_icon')) }}" alt="Icon" width="40">
            </a>
        </div>
        ...
        <div class="sidebar-menu-section">
            ...
            <ul class="sidebar-dropdown-menu parent-menu-list">
                ...
                @if (!isMainCompany())
                    @if (hasFeature('add_ons') &&
                        ...
                        <li class="sidebar-menu-item">
                            <a href="javascript:void(0)"
                                class="parent-item-content has-arrow  {{ menu_active_by_route(['branches.index', 'documents.*', 'document-request.*', 'documents.types.*', 'complain.*', 'verbal-warning.*']) }}">
                            ...
                            <ul
                                    class="child-menu-list  {{ set_active(['documents/*', 'documents/types/*', 'documents/list', 'branches', 'admin/meeting*', 'admin/complain*', 'admin/verbal-warning*']) }}">
                                @if (isModuleActive('EmployeeDocuments'))
                                    @include('employeedocuments::sidebar.user_document_sidebar')
                                @endif

                                {{-- Start write up Routes --}}
                                @if (isModuleActive('WriteUp'))
                                    @include('writeup::sidebar.writeup_sidebar')
                                @endif
                                {{-- End meeting Routes --}}
                    ...
                    @if (!isMainCompany() && isModuleActive('Break'))
                        @include('break::sidebar.break_sidebar')
                    @endif
                    ...
                    @if (hasPermission('attendance_menu') && hasFeature('attendance'))
                        <li class="sidebar-menu-item">
                            ...
                            <ul class="child-menu-list {{ set_active(['hrm/attendance*', 'hrm/qr-code*']) }}">
                                ...
                                @if (hasPermission('attendance_read') && isModuleActive('FaceAttendance'))
                                    <li class="nav-item {{ menu_active_by_route('attendance.face') }}">
                                        <a href="{{ route('attendance.face') }}" target="_blank"
                                           class=" {{ set_active(route('attendance.face')) }}">
                                            <span>{{ _trans('attendance.Face_Attendance') }}</span>
                                        </a>
                                    </li>
                                @endif
                    ...
                    @if (hasPermission('payroll_menu') && hasFeature('payroll'))
                        <li class="sidebar-menu-item {{ set_menu([route('hrm.payroll_items.index')]) }}">
                            ...
                            <ul class="child-menu-list {{ set_active(['hrm/payroll*']) }}">
                                ...
                                @if (hasPermission('salary_list'))
                                    <li
                                            class="nav-item {{ menu_active_by_route(['hrm.payroll_salary.salary-report']) }}">
                                        <a href="{{ route('hrm.payroll_salary.salary-report') }}" class="">
                                            <span> {{ _trans('payroll.Salary Report') }}</span>
                                        </a>
                                    </li>
                                @endif
                                {{-- @if (hasPermission('payroll_set_menu'))
                    ...
                    @if(isModuleActive('Travel') && hasPermission('travel_menu') && hasFeature('travels'))
                        @include('travel::sidebar.travel_sidebar')
                    @endif
                    ...
                    {{-- Start bulk import sidebar --}}
                    @if(isModuleActive('BulkImport') && hasPermission('bulk_upload_menu')  && hasFeature('travels'))
                        @include('bulkimport::sidebar.bulk_import')
                    @endif
                    {{-- End bulk import sidebar --}}
