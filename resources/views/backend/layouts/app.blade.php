...
<body class="default-theme" style="background-image: url(' {{ background_asset(@base_settings('backend_image')) }}')">
    ...
    <input hidden value="{{ _trans('common.Delete') }}" id="delete">
    ...
    <input hidden value="{{ _trans('common.Choose Employee') }}" id="choose_employee_text">
    <input hidden
    ...
    <!-- Firebase  Notification  Script End:--> 

    @if (Route::has('document-request.get-template'))
        <script>

            function updateContent() {
                var document_type_id = $('#document_type').val();
                $.ajax({
                    url: "{{ route('document-request.get-template') }}",
                    type: 'POST',
                    data: {
                        _token: "{{ csrf_token() }}",
                    ...
                    success: function (response) {
                        ...
                        CKEDITOR.instances['editor1Textarea'].setData(response.request_template);
                        // document.getElementById('editor1Textarea').innerHTML=response.request_template;
                    }
                });
            }
        </script>
