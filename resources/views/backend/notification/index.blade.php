                    ...
                    <table class="table table-bordered {{ @$data['class'] }}" id="table">
                        <thead class="thead">
                            <tr>
                                ...
                                <th class="sorting_desc">{{ _trans('common.Seen at') }}</th>
                            </tr>
                        </thead>
                        <tbody>
                            @forelse (allNotification() as $notification)
                                ...
                                <tr class="border-bottom" data-notification_row_id="{{ $notification->id }}">
                                    <td width="50%">
                                        <a href="#" data-notification_id="{{ $notification->id }}"
                                            ...
                                            <div class="notification-content">
                                                <p class="{{ $notification->seen ? 'text-muted' : '' }}">
                                                    {!! @$notification->message !!}</p>
                                                {{-- <h6 class="notification-title font-weight-bold">{{ @$notification->title }}
                                                </h6>
                                                <small class="muted">{!! @$notification->message !!}</small> --}}
                                            </div>
                                        </a>
                                    ...
                                    <td width="25%">
                                        <div class="notification-time text-left">
                                            {{ $notification->seen_at ? $notification->seen_at->diffForHumans() : '' }}

                                            {{-- {{ !blank($notification->seen_at) ? $notification->seen_at->diffForHumans() : '' }} --}}
                                        </div>
                                    </td>
                                </tr>
                            @empty
