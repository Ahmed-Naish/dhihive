...
if (!function_exists('my_asset')) {
    function my_asset($path, $secure = null)
    {
        if ($path == "") {
            ...
            } elseif (env('FILESYSTEM_DRIVER') == 'server' && Storage::disk('s3')->has($path)) {
                return Storage::disk('s3')->url($path);
...
if (!function_exists('settings')) {
    ...
    {
        try {
            if (config('app.single_db') && !in_array(url('/'), config('tenancy.central_domains'))) {
...
function isAttendee()
{
    if (Auth::check()) {
        ...
        $attendance = Attendance::orderByDesc('id')->where($where)
...
{
    return Notification::where(['receiver_id' => auth()->user()->id, 'seen' => false])->latest('id')->take(5)->get();
}
...
{
    return Notification::where(['receiver_id' => auth()->user()->id])->get();
}
        ...
        if (!isMainCompany() && config('app.mood') == 'Saas') {
            return Cache::remember(subdomain() . 'ActiveSubscriptionFeatures', 3600, function () {
                ...
                if (config('app.single_db')) {
                    $featuresKey = SaasSubscription::approved()->runningSubscription()->where('company_id', getCurrentDomainCompany()->id)->first()?->features_key;
                } else {
...
if (!function_exists('getMainCompanyStripeInfo')) {
    ...
    {
        return DB::table('settings')->where(['company_id' => 1, 'name' => $name])->first()?->value;
    }
}
...
if (!function_exists('isExpiredRunningSubscription')) {
    ...
    {
        if (config('app.single_db')) {
            $runningSubscription = SaasSubscription::approved()->runningSubscription()->where('company_id', getCurrentDomainCompany()->id)->first();
        } else {
...
if (!function_exists('isCheckInLate')) {
    function isCheckInLate($startTime, $checkInTime, $considerTime = 0, $isSameDayShiftEnd = true)
    {
        $diffInMinutes = calculateTimeDifference($startTime, $checkInTime, $considerTime, true, $isSameDayShiftEnd);
        $tardyValue = $diffInMinutes >= 120 ? 2 : ($diffInMinutes >= 60 ? 1 : 3);

        return $tardyValue;
    }
}

if (!function_exists('isEarlyCheckOut')) {
    function isEarlyCheckOut($endTime, $checkOutTime, $isSameDayShiftEnd = true, $considerEarlyTime = 0)
    {
        $diffInMinutes = calculateTimeDifference($endTime, $checkOutTime, $considerEarlyTime, false, $isSameDayShiftEnd);
        // return $diffInMinutes >= 120 ? TardyTypeEnum::TwoHoursLate : ($diffInMinutes >= 60 ? TardyTypeEnum::OneHourLate : TardyTypeEnum::ThreeDaysTardy);
        $tardyValue = $diffInMinutes >= 120 ? 2 : ($diffInMinutes >= 60 ? 1 : 3);
        return $tardyValue;
    }
}
...
if (!function_exists('calculateTimeDifference')) {
    function calculateTimeDifference($startTime, $endTime, $considerTime = 0, $isLate = true, $isSameDayShiftEnd = true)
    {
        ...
        if ($isLate) {
            $adjustedStartTime = $startTime->copy()->addMinutes($considerTime);
            if ($endTime->greaterThan($adjustedStartTime)) {
                return $adjustedStartTime->diffInMinutes($endTime);
            }
        } else {
            $adjustedEndTime = $endTime->copy()->subMinutes($considerTime);
            if (!$isSameDayShiftEnd && $startTime->greaterThan($endTime)) {
                $adjustedEndTime->subDay();
            }
            if ($endTime->lessThan($adjustedEndTime)) {
                return $startTime->diffInMinutes($adjustedEndTime);
            }
        }
