...
class DashboardRepository
{
    ...
    public function getApiDashboardStatistics($request)
    {
        try {
            ...
            $screen_data =  AppScreen::where('status_id', 1)->where('company_id', auth()->user()->company_id)->pluck('slug')->toArray();
            if (in_array('appointments', $screen_data)) {
                $data['today'][] = [
                    'image' => global_asset($this->getStatisticsImage('appoinment')),
                    'title' => 'Appointments',
                    'slug' => 'appointment',
                    'number' => Appoinment::query()
