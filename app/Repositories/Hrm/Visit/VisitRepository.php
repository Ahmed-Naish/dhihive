...
class VisitRepository
{
    ...
    public function createNewVisit($request)
    {
        ...
        $params['message'] = "A visit <b class='text-dark'>[" . $visit->title . "]</b> has been created for <b class='text-info'>[" . $visit->date . "]</b> created by <b class='text-warning'>[" . $visit->createdBy->name . "].</b>";

        if (!empty($visit->user->manager_id)) {
            $params['receivers'][] = $visit->user->manager_id;
        }
        $params['forSelf'] = false;
        $params['redirectUrlForWeb'] = route('visit.details', $visit->id);
    ...
    {
        $visit = $this->visit->where('id', $id)->with('schedules','visitImages')->first();
        return $visit;
    }
