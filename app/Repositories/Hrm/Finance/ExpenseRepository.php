...
class ExpenseRepository
{
    ...
    public function store($request)
    {
        ...
        try {
            ...
            $params['message'] = "Expense: <b class='text-warning'>" . $expense->category->name . " [" . $request->amount . "]</b> has been created by <span class='text-dark'>" . auth()->user()->name . '</span>';
            $params['redirectUrlForWeb'] = route('hrm.expenses.show', $expense->id);
            $params['forSelf'] = false;

            if (!empty($expense->user->manager_id)) {
                $params['receivers'][] = $expense->user->manager_id;
            }
