    ...
    public function leaveDetailsInformation($request, $id)
    {
        ...
        if ($user) {
            ...
            if ($leaveRequest) {
                ...
                $data['status'] = @$leaveRequest->status->name == 'Active' ? 'Approve' : @$leaveRequest->status->name;
                ...
                $data['attachment_type'] = @$leaveRequest->attachment->type;
        ...
        try {
            ...
            if ($userDepartment) {
                ...
                if ($leaveType) {
                    ...
                    $substitute = null;
                    if($request->filled('substitute_id')){
                        $substitute = $request->substitute_id;
                    }
                    if ($request->substitute_id === 'null'){
                        $substitute  = null;
                    }
                    ...
                    if ($user_check || $department_check) {
                        ...
                        if ($user) {
                            ...
                            $leaveRequest->substitute_id = $substitute;
                            ...
                            if ($request->hasFile('attachment')) {
                                $uploadFile = $this->uploadImage($request->attachment, 'uploads/employeeDocuments');
                                $leaveRequest->attachment_file_id = $uploadFile->id;
                                $leaveRequest->image_url = asset($uploadFile->img_path);
                            } else {
                                $leaveRequest->attachment_file_id = null;
                            }
                            ...
                            if($leaveRequest->save()){
                                (new NotificationService)->storeLeaveRequestNotification($leaveRequest);

                                return $this->responseWithSuccess('Leave request has been created', [], 200);
                            }else {
                                return $this->responseWithError('Leave request could not be created', [], 500);
                            }
                        } else {
    ...
    public function approveOrRejectOrCancel($id, $status): bool
    {
        ...
        if ($leaveRequest) {
            ...
            if ($status == 1) {
                ...
                $forReceiverMessage = '<b>' . $leaveRequest->user->name . '\'s</b> leave request has been <span class="text-danger">Approved</span> by <b class="text-warning">' . auth()->user()->name . '</b>';
                $redirectUrlForWeb = route('leaveRequest.index');
                $redirectUrlForApp = 'leave_request';

                $data['leaveRequest'] = $leaveRequest;

                (new NotificationService)->storeNotification($forReceiverMessage, '', $redirectUrlForWeb, $redirectUrlForApp, true, $data);
                ...
                $forReceiverMessage = '<b>' . $leaveRequest->user->name . '\'s</b> leave request has been <span class="text-danger">cancelled</span> by <b class="text-warning">' . auth()->user()->name . '</b>';
                $redirectUrlForWeb = route('leaveRequest.index');
                $redirectUrlForApp = 'leave_request';
...
//                }
                ...
                (new NotificationService)->storeLeaveRequestRejectNotification($leaveRequest);
                $this->rejectedBy($leaveRequest);
            }
            return true;
        } else {
    ...
    public function storeDailyLeave($request)
    {
        ...
        $daily_leave->status_id = 2;
        // $daily_leave->save();
        $formated_leave_type = str_replace('_', ' ', Str::title($daily_leave->leave_type));
        if ($daily_leave->save()) {

            $author = $this->createdBy($daily_leave);
            $daily_leave->author_info_id = $author->id;
            $daily_leave->save();
    ...
    public function staffListView($request)
    {
        ...
        try {
            ...
            } elseif ($my_team_members) {
                $leaveRequests->whereIn('user_id', array_merge($my_team_members, [auth()->id()]));
            }
        ...
        try {
            ...
            return [
                'data' => $data->map(function ($data) {
                    ...
                    $filePath = $data->attachment_file_id ? uploaded_asset($data->attachment_file_id) : $data->image_url;

                    return [

                        'id' => $data->id,
                        'file' => $filePath ? '<a href="' . $filePath . '" download class="btn btn-white btn-sm"><i class="fas fa-download"></i></a>' : _trans('common.No File'),
                        'name' => @$data->user->name,
