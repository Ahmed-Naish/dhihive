...
class MeetingRepository
{
    ...
    public function store($request): \Illuminate\Http\JsonResponse
    {
        ...
        $meeting->start_at = main_time_format($request->start_at);
        $meeting->end_at = main_time_format($request->end_at);
        ...
        if($request->participants){
            $user_ids = array_diff(explode(',', $request->participants), [auth()->id()]);
            $user_ids = array_values($user_ids);
        }elseif(in_array(0, $request->department_id)){
            $user_ids = User::where('status_id', 1)->where('id', '!=', auth()->id())->where('company_id', auth()->user()->company_id)->whereNotIn('role_id',[1,2,3])->pluck('id')->toArray();
        }elseif($request->department_id){
            $user_ids = User::where('status_id', 1)->where('id', '!=', auth()->id())->whereIn('department_id', $request->department_id)->where('company_id', auth()->user()->company_id)->whereNotIn('role_id',[1,2,3])->pluck('id')->toArray();
        }
        ...
        if ($request->participants) {
            ...
            $params['message'] = $meeting->title;
            $params['receivers'] = $user_ids;
            $params['redirectUrlForWeb'] = "";
    ...
    public function newStore($request)
    {
        ...
        try {
            ...
            if ($request->hasFile('attachment')) {
                $uploadedFile = $this->uploadImage($request->attachment, 'uploads/meeting');
                $meeting->attachment_file_id = @$uploadedFile->id;
                $meeting->image_url = @$uploadedFile->img_path;
            }
            $meeting->status_id = 1;
            $meeting->save();
            ...
            if (@$request->user_id) {
                ...
                $user_ids = array_diff($user_ids, [auth()->id()]);
                $user_ids = array_values($user_ids);
...
//                    $this->sendCustomNotification($participant, $details, $notification_for);
                ...
                $params['message'] = "A meeting <b class='text-warning'>(" . $request->title . ")</b> has been created by <span class='text-dark'>" . auth()->user()->name . '</span>';
                $params['receivers'] = $user_ids;
                $params['redirectUrlForWeb'] = route('meeting.view', $meeting->id);
                $params['forSelf'] = false;
    ...
    public function newUpdate($request, $id)
    {
        ...
        try {
            ...
            if($request->user_id){
                ...
                $user_ids = User::where('status_id', 1)->where('company_id', auth()->user()->company_id)->whereNotIn('role_id',[1,2,3])->pluck('id')->toArray();
            }
            elseif($request->department_id){
                $user_ids = User::where('status_id', 1)->whereIn('department_id', $request->department_id)->where('company_id', auth()->user()->company_id)->whereNotIn('role_id',[1,2,3])->pluck('id')->toArray();
            }
            ...
            $params['message'] = "A meeting <b class='text-warning'>(" . $request->title . ")</b> has been updated by <span class='text-dark'>" . Auth::user()->name . '</span>';
            $params['receivers'] = $user_ids;
            $params['redirectUrlForWeb'] = route('meeting.view', $meeting->id);
            $params['forSelf'] = false;
            storeNotification($params);
