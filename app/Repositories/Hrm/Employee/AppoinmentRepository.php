...
class AppoinmentRepository
{
    ...
    public function store($request)
    {
        ...
        $appoinment_participant->company_id = auth()->user()->company_id;
        $appoinment_participant->branch_id = auth()->user()->branch_id;
        $appoinment_participant->save();

        $user_ids[] = $appoinment_participant->participant_id;
        //Another Participant
        $appoinment_participant = new AppoinmentParticipant;
...
//        }
        ...
        $params['message'] = "An Appoinment <b class='text-dark'>[" . $request->title . "]</b> has been created by <span class='text-warning'>" . Auth::user()->name . "</span>. Appointment at <b class='text-primary'>" . Carbon::parse($request->date)->format('Y-m-d') . " [" . Carbon::parse($request->appoinment_start_at)->format('h:i:A') . ' - ' . Carbon::parse($request->appoinment_end_at)->format('h:i:A') . "]</b>";
        ...
        return [
            'data' => $files->map(function ($data) {
                return [
                    ...
                    'start_time'      => Carbon::parse($data->appoinment_start_at)->format('h:i A'),
                    'end_time'        => Carbon::parse($data->appoinment_end_at)->format('h:i A'),
