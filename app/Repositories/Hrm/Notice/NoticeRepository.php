           ...
           $notice->save();
            ...
            if (in_array(0, $request->department_id)) {
                ...
                foreach ($users as $user) {
                    ...
                    // Send Data base notification
                    $params['message'] =  '<b class="text-danger">' . $notice->subject . ':</b> <span class="text-dark">' . @$notice->createdBy->name . ' [' . @$notice->createdBy->employee_id . ']</span> has created a notice';

                    $params['redirectUrlForWeb'] = route('notice.detail', $notice->id);
...
//                        $this->sendCustomNotification($user, $details,$notification_for);
                        (new NotificationService)->storeNoticeNotification($notice, $user);
                    }
...
//                        (new PushNotificationService)->push('department'.$department->id, $request->subject, $request->description,$notice->attachment_file_id?uploaded_asset($notice->attachment_file_id): null );
//                    }
                    ...
                    foreach ($department->users as $user) {
                        ...
                        (new NotificationService)->storeNoticeUpdateNotification($notice, $user);
                    }
                }
    ...
    function table($request)
    {
        ...
        return [
            'data' => $data->map(function ($data) {
                $action_button = '';
                if (hasPermission('notice_update')) {
                    $action_button .= '<a href="' . route('notice.edit', $data->id) . '" class="dropdown-item">' . _trans('common.Edit') . '</a>';
                }
                // if (hasPermission('notice_update')) {
                //     $action_button .= '<a href="' . route('notice.statistics', $data->id) . '" class="dropdown-item">' . _trans('common.Statistics') . '</a>';
                // }
                if (hasPermission('designation_delete')) {
                    $action_button .= actionButton(_trans('common.Delete'), '__globalDelete(' . $data->id . ',`dashboard/announcement/notice/delete/`)', 'delete');
                }
