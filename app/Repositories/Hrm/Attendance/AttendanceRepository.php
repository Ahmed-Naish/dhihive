...
class AttendanceRepository
{
    ...
    public function getCheckInCheckoutStatus($userId)
    {
        ...
        if ($user) {
            ...
            $attendance = $this->attendance->query()->orderByDesc('id')->where($where)
...
//                        'stay_time' => $stay_time
    ...
    public function qrStatus($request)
    {
        ...
        if (settings('attendance_method') == 'QR') {
            ...
            try {
                $qrCode = decrypt($request->qr_scan);
                $company_id = auth()->user()->company_id;
                $actualQrCode = request()->getSchemeAndHttpHost() . '-' . $company_id;

                if ($qrCode == $actualQrCode) {
...
//                    if ($check_in->in_status == AttendanceStatus::LATE){
//                        (new NotificationService)->storeLateCheckInNotification($check_in->late_time);
//                    }
...
//                            if ($check_in->out_status == AttendanceStatus::LATE){
//                                (new NotificationService)->storeEarlyCheckoutNotification($check_in->late_time);
//                            }
...
//                            if($check_in->out_status == AttendanceStatus::LEFT_EARLY){
//                                (new NotificationService)->storeEarlyCheckoutNotification($check_in->late_time);
//                            }
...
//            $stay_time = timeDiff($check_in, $check_out_time, 'all', $start_date = null, $end_date = null);
			...
			$stay_time = timeDiff($check_in, (strtotime($check_in) > strtotime($check_out_time) ? date('Y-m-d H:i:s', strtotime($check_out_time . ' +1 day')) : $check_out_time), 'all', $start_date = null, $end_date = null);
        ...
        try {
            ...
            if ($user) {
                ...
                if (count($attendance_status) > 0) {
                    ...
                    $check_out = $this->attendance->query()->where('id', $id)->where('shift_id', $shiftId)->first();
