        ...
        return datatables()->of($content)
            ->addColumn('action', function ($data) {
                ...
                if (hasPermission('project_delete') && @$data->payment != 8) {
                    $action_button .= actionButton(_trans('common.Delete'), '__globalDelete(' . $data->id . ',`admin/project/delete/`)', _trans('common.Delete'));
                }
        ...
        try {
            ...
            if (@$request->user_ids) {
                foreach ($request->user_ids as $user_id) {
                    ...
                    $params['message'] = "<b class='text-dark'>" . $member_info->name . ' [' . $member_info->employee_id . "]</b> have been assigned the project <b class='text-dark'>[" . $project->name . "]</b> with a priority level of <b class='text-warning'>[" . $project->priorityStatus->name . "].</b>";
                    $params['receivers'][] = $user_id;

                    if (!empty($member_info->manager_id)) {
                        $params['receivers'][] = $member_info->manager_id;
                    }

                    $params['redirectUrlForWeb'] = route('project.view', [$project->id, 'details']);
