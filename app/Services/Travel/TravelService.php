    ...
    function store($request)
    {
        ...
        try {
            ...
            if (auth()->user()->role_id < 4) {
                $params['message'] = "<span class='text-dark'>The " . auth()->user()->name . ' [' . auth()->user()->employee_id . "]</span> has created a travel itinerary for <b class='text-dark'>" . @$travel->user->name . ' [' . @$travel->user->employee_id . "]</b> to <b class='text-warning'>" . $travel->type->name . ".</b>";
                $params['receivers'] = [$request->user_id];

                if (!empty($travel->user->manager_id)) {
                    $params['receivers'][] = $travel->user->manager_id;
                }
            } else {
                $params['message'] = "<span class='text-dark'>" . @$travel->user->name . ' [' . @$travel->user->employee_id . "]</span> has created a travel itinerary for <b class='text-warning'>" . $travel->type->name . ".</b>";
            ...
            $params['redirectUrlForWeb'] = route('travel.view', $travel->id);
